# Template — Runtime Specification

A normative, implementation-independent template of the Runtime library. Copy it to author
a Runtime specification; every section must be populated (no placeholders) to pass the
...
5. **Runtime Content** — the Runtime Specifications, Runtime Responsibilities, Runtime Constraints, Execution Models … this specification defines.
...
- Declare the single owning authority (Runtime Architect) and the owning chapter
  ([Chapter 07](../../../handbook/07-runtime-platform/CHAPTER.md)).
